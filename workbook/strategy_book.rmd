...
Optimal k for long trades is 3, and optimal k for short trades is 5, using the elbow method.

```{r}
long_clusters <- kmeans_analysis(long_trades, k = 3)
short_clusters <- kmeans_analysis(short_trades, k = 5)
```
...
color_gradient = c("red","orange","gold","green", "darkgreen")
cluster_summary <- aggregate(cbind(delta, moneyness, implied_volatility, rho) ~ cluster, 
                             data = short_clusters, FUN = mean)
...
short_clusters$profit_rate <- cluster_summary$profit_rate[match(short_clusters$cluster, cluster_summary$cluster)]
